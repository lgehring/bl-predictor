...
def test_gui():
    ...
    assert (tested_gui.date_label.cget("text") == "Choose a period "
                                                  "of "
                                                  "time:")
    # timeframe slider
    assert (tested_gui.slider.W == 400)
    ...
    assert (tested_gui.model_label.cget("text") == "Choose a "
                                                   "prediction "
                                                   "model:")
    assert (tested_gui.train_ml_button.cget("text") == "Train "
                                                       "prediction "
                                                       "model")
    tested_gui._train_model_helper()
    assert (tested_gui.train_ml_button.cget("text") == 'Model '
                                                       'trained')
